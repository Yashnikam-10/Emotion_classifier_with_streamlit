...
def add_page_visited_details(pagename,timeOfvisit, c, conn):
	c.execute('INSERT INTO pageTrackTable(pagename,timeOfvisit) VALUES(?,?)',(pagename,timeOfvisit))
	conn.commit()

def view_all_page_visited_details(c):
...
def create_emotionclf_table(c):
	c.execute('CREATE TABLE IF NOT EXISTS emotionclfTable(rawtext TEXT,prediction TEXT,probability DECIMAL(6,5),timeOfvisit TIMESTAMP)')

def add_prediction_details(rawtext,prediction,probability,timeOfvisit, c, conn):
...
def view_all_prediction_details(c):
